...
class _TransformerDecoderSingleEncoding(_TransformerEncoder):
    """
    A transformer decoder that applies cross attention to the same conditioning encoding
    in all decoder layers
    """
    def forward(self, src, t, cond):
        x = src
        for layer in self.layers:
            x = layer(x, t, cond)  # use the first condition for all layers
        return x

class _DitNoiseDecoder(nn.Module):
    """
    A decoder-only DiT noise network that can be used for diffusion
    tasks where the input is a sequence of noisy actions and the output
    is the predicted noise for those actions.
    """
    def __init__(
        self,
        ac_dim,
        ...
        self.decoder = _TransformerDecoderSingleEncoding(decoder_module, num_blocks)
    ...
    def forward(
        ...
        """
        Forward pass of the DiT noise decoder.

        Args:
            noisy_actions [B, P, A]: Noisy actions tensor where B is batch size,
                P is the action chunk length, and A is the action dimension.
            time [B]: Tensor of timesteps for each action in the batch.
            obs_enc [B, T, C]: Observation encoding used to condition the decoder.
                T is the number of observation tokens and C is the feature dimension.
        """
        time_enc = self.time_net(time)
        
        ac_tokens = self.ac_proj(noisy_actions)
        # Switch to (S, B, D) format for transformer decoder
        obs_enc = einops.rearrange(obs_enc, "b s c -> s b c")
        ac_tokens = einops.rearrange(ac_tokens, "b s ... -> s b ...")
        dec_in = ac_tokens + self.dec_pos

        # apply decoder
        dec_out = self.decoder(dec_in, time_enc, obs_enc)

        # apply final epsilon prediction layer
        return self.eps_out(dec_out, time_enc, obs_enc)

if __name__ == "__main__":
    from diffusers.schedulers.scheduling_ddim import DDIMScheduler

    # Example usage of the DiT noise network
    noise_net = _DitNoiseDecoder(
        ac_dim=2,
        ac_chunk=10,
    )
    noise_actions = torch.randn(5, 10, 2)  # batch of 5, 10 actions each with 2 dimensions
    time = torch.randint(0, 1000, (5,))  # random timesteps for each action
    obs_enc = torch.randn(10, 5, 512)  # 10 tokens, batch of 5, each token of dimension 512
    noise_pred = noise_net(noise_actions, time, obs_enc)
    print(noise_pred.shape)  # should be (10, 5, 2) for the output of the noise prediction

    # Test inference with the noise network
    eval_steps = 10  # number of steps for inference
    ...
    diffusion_schedule.set_timesteps(eval_steps)
    diffusion_schedule.alphas_cumprod = (
        diffusion_schedule.alphas_cumprod.to(noise_actions.device)
    )
    for timestep in diffusion_schedule.timesteps:
        # predict noise given timestep
        batched_timestep = timestep.unsqueeze(0).repeat(noise_actions.shape[0]).to(noise_actions.device)
        noise_pred = noise_net.forward(noise_actions, batched_timestep, obs_enc)

        # take diffusion step
        noise_actions = diffusion_schedule.step(
            model_output=noise_pred, timestep=timestep, sample=noise_actions
        ).prev_sample

    print(noise_actions.shape)  # should be (10, 5, 2) for the final actions after diffusion
